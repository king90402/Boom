...
// --------- Clase contenedora de todos los metodos de validacion del sistema
...
public class Validaciones {
    ...
    private static final Pattern EMAIL_PATTERN = Pattern.compile(
        "^(?=.{6,254}$)" +                    // Longitud total 6-254
        "(?=.{1,64}@)" +                      // Usuario max 64 chars
        "(?!.*\\.\\.)" +                      // No puntos consecutivos
        "(?!.*__)" +                          // No guiones bajos consecutivos
        "(?!.*--)" +                          // No guiones consecutivos
        "[A-Za-z0-9]" +                       // Inicia con alfanumerico
        "[A-Za-z0-9._%+-]{0,62}" +            // Cuerpo del usuario
        "[A-Za-z0-9]" +                       // Termina con alfanumerico
        "@" +
        "(?=.{1,255}$)" +
        "(?:[A-Za-z0-9]" +                    // Dominio inicia con alfanumerico
        "(?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?" +
        "\\.)+" +
        "[A-Za-z]{2,63}$"                     // TLD 2-63 chars
    );
    
    // Patron para validar ID/Documento - Solo numeros, entre 7 y 12 digitos

    private static final Pattern ID_PATTERN = Pattern.compile(
        "^[0-9]{7,12}$"
    );
    
    // Patron para validar celular - Puede iniciar con +, luego 10-15 digitos

    private static final Pattern PHONE_PATTERN = Pattern.compile(
        "^\\+?[0-9]{10,15}$"
    );
    
    // Patron para validar nombres y apellidos - Solo letras (incluyendo tildes) y espacios 
    ...
    // Valida que un numero sea no negativo
 
    public static boolean esNoNegativo(double numero) {
        return numero >= 0;
    }
    
    // Valida que un precio sea valido (mayor que 0)
    ...
    // Valida que una cantidad sea valida (entero >= 0)
    ...
    public static String[] getDominiosValidos() {
        return DOMINIOS_VALIDOS.clone();
    }
